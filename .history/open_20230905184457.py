...
# 指定要监听的程序名称
target_program_name = "QQ.exe"  # 请根据你要监听的程序名称进行修改


def is_program_open_and_focused(program_name):
    # 获取当前活动窗口的标题
    current_window_title = gw.getActiveWindow().title

    # 检查当前活动窗口是否包含指定程序名称，并且检查指定程序是否正在运行
    return program_name in current_window_title and is_process_running(program_name)


def is_process_running(process_name):
    print(psutil.process_iter(attrs=['pid', 'name']))
...
is_process_running(target_program_name)

while True:
    # 检查指定程序是否处于打开且窗口界面
    if is_program_open_and_focused(target_program_name):
